  ...
  const calculators = [
    {
      ...
      callback: ({x, n, p}) => ((factorial(n)/(factorial(x)*factorial(n-x)))*(Math.pow(p, x)*Math.pow((1-p), n-x))).toFixed(4)
    },
    {
      ...
      callback: ({ N, r, n, x }) => ((ncr(r, x)*ncr(N-r,n-x))/ncr(N, n)).toFixed(4)
    },
    {
      ...
      callback: ({ x, m }) => (((Math.pow(m, x))/factorial(x))*Math.exp(-m)).toFixed(4)
    },
    {
      ...
      callback: ({ m, s, x}) => ((1/(s*Math.sqrt(2*Math.PI)))*Math.exp(-Math.pow(x-m, 2)/(2*s*s))).toFixed(4)
    },
    {
      ...
      params: [["Media", "m"], ["Desviación estándar", "s"], ["Variable", "x"]],
      callback: ({ m, s, x }) => ((1/Math.sqrt(2*Math.PI))*Math.exp(1)*((x-m)*-1/s)/2).toFixed(4)
    },
    {
